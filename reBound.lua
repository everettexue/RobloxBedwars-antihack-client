        ...
        errEvent=service.ScriptContext.Error:connect(function(Message, Trace, Script)
            if not kicking and (not Script or ((not Trace or Trace==""))) then
                ...
                if continue then
                    ...
                    client.Remote("Exploited","kick","Exploit detected. Traceless/Scriptless script.")
                    localplayer:Kick(tostring((Script and Script:GetFullName()) or Script).. " | "..tostring(Trace))errEvent:disconnect() --Seems to have problems sometimes. Hopefully people who get wrongfully kicked will send a screenshot.
                    kicking=true
                end
            end
        end)--]]
